#ifndef GBOPT_CMDLINE_HPP_INCLUDED
#define GBOPT_CMDLINE_HPP_INCLUDED
...
struct CmdLine
{
    ...
    auto set_args(std::span<char const*> val) noexcept -> void { args_ = val; }
                   ...
                   Allocator alloc = Allocator {}) -> CmdLine<Id, Allocator>
{
    ...
    auto parts = split_argv_list(args);

    auto pos = args.begin();
    auto mid = std::get<0>(parts);
    auto last = mid;
    ...
    output.set_args(
        { pos != std::get<1>(parts) ? &*pos : nullptr,
          static_cast<std::size_t>(std::distance(pos, std::get<1>(parts))) });
...
auto print_flag_defs(Id const (&defs)[N]) noexcept -> void
requires(N > 0)
{
    print_flag_defs(std::span<Id const> { &defs[0], N });
}

} // namespace gbopt
#endif // GBOPT_CMDLINE_HPP_INCLUDED
